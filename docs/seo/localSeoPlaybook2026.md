...
2. **Ship service x city money pages.** `/web-design-bakersfield` + `/seo-bakersfield` (flat exact-match URLs like every ranking competitor), new MoneyPage template. `/services/bakersfield` upgraded into the local hub linking down. This is the #1-ranked organic factor executed literally.
...
- **L2 - Money pages (Claude builds, Rahul reviews):** MoneyPage template + `/web-design-bakersfield` + `/seo-bakersfield` per section 3 anatomy: answer-first intro, case-study metrics, founder section, pricing ranges, 8-10 PAA-targeted FAQs, embedded map, city reviews block (grows as reviews arrive), Service + FAQPage + breadcrumb schema referencing #business/#rahul, prerender + sitemap. Upgrade `/services/bakersfield` to hub (fix "primarily national" copy). GBP website link -> hub. Design pass via emilDesignEng; copy via voice DNA + humanizer.
...
- GBP: website link -> `/services/bakersfield` hub; widen listed hours (openness factor); confirm primary category stays "Marketing agency" vs "Website designer" (recommend keeping Marketing agency; category chase can wait for data).
